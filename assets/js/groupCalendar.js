$(document).ready(function() {
	...
	var calendar=$('#calendar').fullCalendar({
		...
		insertView: $.post('/index.php/calendar/calView',{user:ugroup},function(data){
			a=data.split("<br>")
			j=0;
		...
		select: function(start, end) {
			...
			var se=window.open('/index.php/calendar/select?start='+String(start)+'&end='+String(end),'','toolbar=no,status=no,resizable=no,menubar=no,directories=no,width=200, height=50,top='+top+',left='+left);
			 ...
			 eventResize:function(event,jsEvent,ui,view){
			 	...
			 	$.post('/index.php/calendar/calUpdate',{id:event.id,start:String(event.start),end:String(event.end)})
			 },
...
function select(input){
	...
	$.post('/index.php/calendar/calInsert',{title:ti,start:String(start),end:String(end),color:co,user:ugroup},function(data){
